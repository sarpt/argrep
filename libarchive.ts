...
export class LibArchive {
  ...
  open(archivePath: string): Deno.UnsafePointer | undefined {
    ...
    // needed for single files compressed using .xz
    this.lib.symbols.archive_read_support_compression_all(archive);
    this.lib.symbols.archive_read_support_format_raw(archive); 
  ...
  listFiles(archivePath: string): string[] {
    ...
    while (this.lib.symbols.archive_read_next_header(archive, archive_entry_address) === ARCHIVE_RESULTS.OK) {
